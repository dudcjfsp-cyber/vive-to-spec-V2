...
2) Demo 모드(기존: 사용자 API 키 직접 입력)
```bash
npm run dev -- --host 127.0.0.1 --port 5173 --strictPort
```

3) Managed 모드(신규: 사용자 API 키 입력 없음)

PowerShell 예시:
```powershell
$env:GEMINI_API_KEY="<server_key>"
$env:OPENAI_API_KEY="<server_key>"
$env:ANTHROPIC_API_KEY="<server_key>"

npm run serve:managed-api
```

새 터미널에서 프론트 실행:
```powershell
$env:VITE_APP_MODE="managed"
$env:VITE_MANAGED_API_BASE_URL="http://127.0.0.1:8787/api"
npm run dev -- --host 127.0.0.1 --port 5173 --strictPort
```
...
- Vercel route settings live directly in `api/*.js`
